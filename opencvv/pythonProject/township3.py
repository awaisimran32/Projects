...
strawberry_img=cv2.imread("used/strawbery.jpg")
wheat_img=cv2.imread("used/wheat.jpg")
...
strawberry_gray_img = cv2.cvtColor(strawberry_img, cv2.COLOR_BGR2GRAY)
...
for scale in scales:
    ...
    resized_template3=cv2.resize(strawberry_gray_img,None,fx=scale,fy=scale,interpolation=cv2.INTER_LINEAR)
    ...
    result3=cv2.matchTemplate(gray_img,resized_template3,cv2.TM_CCOEFF_NORMED)

    yloc, xloc = np.where(result >= threshold)
    yloc2, xloc2 = np.where(result2 >= threshold)
    yloc3,xloc3=np.where(result3>=threshold)
    ...
    for(x,y) in zip(xloc3,yloc3):
        rectangles3.append([int(x),int(y),int(resized_template3.shape[1]),int(resized_template3.shape[0])])
...
rectangles3, weight3 = cv2.groupRectangles(rectangles3, 1, 0.2)


print(f"Number of rectangles: {len(rectangles)}")
print(f"Number of rectangles: {len(rectangles2)}")
print(f"Number of rectangles: {len(rectangles3)}")
...
for (x, y, w, h) in rectangles3:
    # Draw the rectangle
    cv2.rectangle(farm_img, (x, y), (x + w, y + h), (255,0,0), 2)
    ...
    font_scale3 = 0.6
    font_thickness3 = 1
    text_color3 = (0, 255, 255)  # Same color as the rectangle
    text_size3 = cv2.getTextSize(label3, font3, font_scale3, font_thickness3)[0]

    # Position the text above the rectangle
    text_x3 = x
    text_y3 = y - 10  # Slightly above the rectangle
    if text_y3 < 0:  # Ensure the text stays within bounds
        text_y3 = y + 20

    cv2.putText(farm_img, label3, (text_x3, text_y3), font3, font_scale3, text_color3, font_thickness3)
